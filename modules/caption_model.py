...
class CaptionModel(nn.Module):
    ...
    def forward(self, *args, **kwargs):
        ...
        # mask the following, as input for forward & sample is different
        # if 'mode' in kwargs:
        #     del kwargs['mode']
    ...
    def beam_search(self, init_state, init_logprobs, *args, **kwargs):
        ...
        def beam_step(logprobs, unaug_logprobs, beam_size, t, beam_seq, beam_seq_logprobs, beam_logprobs_sum, state, aux_info_table=None):
            # INPUTS:
            ...
            # aux_info_table : auxiliary data

            batch_size = beam_logprobs_sum.shape[0]
            vocab_size = logprobs.shape[-1]
            logprobs = logprobs.reshape(batch_size, -1, vocab_size)  # NxbxV
            aux_info = [t.reshape(batch_size, beam_size, t.shape[1], -1) for t in aux_info_table]
            if t == 0:
            ...
            if t > 0:
                ...
                aux_info = [t.gather(1, beam_ix.unsqueeze(-1).unsqueeze(-1).expand_as(t)) for t in aux_info]
            aux_info = [t.reshape(-1, t.shape[-2], t.shape[-1]) for t in aux_info]
            ...
            state = new_state
            return beam_seq, beam_seq_logprobs, beam_logprobs_sum, state, aux_info
        ...
        aux_info_table = [[torch.zeros((args[-1].shape[0], 14, 1), device=args[-1].device), args[-1]] for _ in
                          range(group_size)]
        # END INIT
        ...
        for t in range(self.max_seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.max_seq_length + divm - 1:
                    ...
                    state_table[divm], \
                    aux_info_table[divm] = beam_step(logprobs,
                                                  unaug_logprobs,
                                                  bdash,
                                                  t - divm,
                                                  beam_seq_table[divm],
                                                  beam_seq_logprobs_table[divm],
                                                  beam_logprobs_sum_table[divm],
                                                  state_table[divm], aux_info_table[divm])
                        ...
                        for vix in range(bdash):
                            if is_end[vix]:
                                final_beam = {
                                    ...
                                    'others': [aux_info_table[divm][0].reshape(batch_size, bdash, aux_info_table[divm][0].shape[-2], -1)[b, vix, :, -1],
                                               aux_info_table[divm][1].reshape(batch_size, bdash, aux_info_table[divm][1].shape[-2], -1)[b, vix]]
                                }
                    ...
                    logprobs_table[divm], state_table[divm], tmp_info_table = self.get_logprobs_state(it.cuda(), *(
                            args[divm][:-1] + [state_table[divm]]))
                    aux_info_table[divm][0] = torch.concat((aux_info_table[divm][0], tmp_info_table[0].unsqueeze(-1)), dim=2)
                    aux_info_table[divm][1] = torch.concat((aux_info_table[divm][1], tmp_info_table[1]), dim=2)
        ...
        for t in range(self.max_seq_length + group_size - 1):
            for divm in range(group_size):
                if t >= divm and t <= self.max_seq_length + divm - 1:
                    ...
                    logprobs_table[divm], state_table[divm], _ = self.get_logprobs_state(it.cuda(), *(
