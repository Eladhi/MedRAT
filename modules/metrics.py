...
def compute_scores(gts, res, return_list=False, return_list_metric="BLEU_4"):
    """
    ...
    for scorer, method in scorers:
        ...
        if type(method) == list:
            for i, (sc, m) in enumerate(zip(score, method)):
                eval_res[m] = sc
                if return_list and return_list_metric == m:
                    out_list.append(scores[i])
        else:
            eval_res[method] = score
            if return_list and return_list_metric == method:
                out_list.append(scores)
    if return_list:
        return eval_res, out_list
    return eval_res
