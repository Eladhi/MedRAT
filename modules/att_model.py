...
class AttModel(CaptionModel):
    ...
    def _prepare_feature(self, fc_feats, att_feats, att_masks):
        ...
        return fc_feats, att_feats, p_att_feats, att_masks, None

    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, prob_attn, state, output_logsoftmax=1):
        # 'it' contains a word index
        xt = self.embed(it)

        output, state, aux_info = self.core(xt, fc_feats, att_feats, p_att_feats, state, att_masks, prob_attn)
        if output_logsoftmax:
            logprobs = F.log_softmax(self.logit(output), dim=1)
        else:
            logprobs = self.logit(output)

        return logprobs, state, aux_info
    ...
    def _sample_beam(self, fc_feats, att_feats, att_masks=None, opt={}):
        ...
        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, prob_attn_fe = self._prepare_feature(fc_feats, att_feats, att_masks)
        ...
        others_out = [[] for _ in range(batch_size)]

        state = self.init_hidden(batch_size)

        # first step, feed bos
        it = fc_feats.new_full([batch_size], self.bos_idx, dtype=torch.long)
        logprobs, state, aux_info = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, prob_attn_fe, state)

        attn_0 = aux_info[1]

        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, p_attn_fe, p_attn_0 = utils.repeat_tensors(beam_size,
                                                                                  [p_fc_feats, p_att_feats,
                                                                                   pp_att_feats, p_att_masks,
                                                                                   prob_attn_fe, attn_0]
                                                                                  )
        self.done_beams = self.beam_search(state, logprobs, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, p_attn_fe, p_attn_0, opt=opt)
        for k in range(batch_size):
            if sample_n == beam_size:
                for _n in range(sample_n):
                    ...
                    others_out[k] = self.done_beams[k][_n]['others']
            else:
                seq_len = self.done_beams[k][0]['seq'].shape[0]
                seq[k, :seq_len] = self.done_beams[k][0]['seq']  # the first beam has highest cumulative score
                seqLogprobs[k, :seq_len] = self.done_beams[k][0]['logps']
                others_out[k] = self.done_beams[k][0]['others']
        # return the samples and their log likelihoods
        return seq, seqLogprobs, others_out

    def _sample(self, fc_feats, att_feats, att_masks=None, update_opts={}, mode='sample'):
        ...
        p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, prob_attn = self._prepare_feature(fc_feats, att_feats, att_masks)
        ...
        for t in range(self.max_seq_length + 1):
            ...
            logprobs, state, mlc_pred = self.get_logprobs_state(it, p_fc_feats, p_att_feats, pp_att_feats, p_att_masks, prob_attn,
        ...
        return seq, seqLogprobs, mlc_pred
