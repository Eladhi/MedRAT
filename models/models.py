...
class BaseCMNModel(nn.Module):
    ...
    def forward_iu_xray(self, images, targets=None, mode='train', update_opts={}):
        ...
        if mode == 'train':
            ...
            output, output_probs, mlc_pred = self.encoder_decoder(fc_feats, att_feats, mode='sample', update_opts=update_opts)
            return output, output_probs, mlc_pred
        else:
            raise ValueError

    def forward_mimic_cxr(self, images, targets=None, mode='train', update_opts={}):
        if images.dim() == 5 and images.shape[1] == 2:  # when iu-xray is a secondary dataset for evaluation
            att_feats_0, fc_feats_0 = self.visual_extractor(images[:, 0])
            att_feats_1, fc_feats_1 = self.visual_extractor(images[:, 1])
            fc_feats = torch.cat((fc_feats_0, fc_feats_1), dim=1)
            att_feats = torch.cat((att_feats_0, att_feats_1), dim=1)
